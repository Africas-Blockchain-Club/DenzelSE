...
contract Azuki is ERC721A {    address public owner;
    ...
    function mint(address to) external payable {
        ...
        if (countReserved<10 && isReserved(to)) {            
            require(msg.value == 1 ether, "Insufficient funds. funds must be greater than 1");            require(!addressOwnsNFT(to), "one token one address");            _mint(to, 1);            countReserved++;
        }
        else{
            require(msg.value == 1 ether, "Insufficient funds. funds must be greater than 1");            require(totalSupply() < MaxSupply, "Exceeded amount of NFT's mintable.");            require(!addressOwnsNFT(to), "one token one address");            _mint(to, 1);
        }    
    }    
